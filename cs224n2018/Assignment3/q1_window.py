...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        for i in range(len(sentence)):
            if i >= window_size:               
                sublist = sentence[i - window_size:i]         
            else:           
                sublist = [start]*(window_size - i)+ sentence[:i]                 
            if (len(sentence) - i -1) >= window_size:                
                sublist += sentence[i:i+window_size+1]
            else:
                right = sentence[i:len(sentence)] + [end]*(window_size + i + 1 - len(sentence))
                sublist += right
            flat_list = [item for l in sublist for item in l]           
            windowed_data.append((flat_list,labels[i]))      
...
class WindowModel(NERModel):
    ...
    def add_loss_op(self, pred):
        ...
        total_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=pred,labels=self.labels_placeholder) 
