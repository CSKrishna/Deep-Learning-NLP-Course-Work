...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        init = tf.constant_initializer(self.pretrained_embeddings)       
        word_embeddings = tf.get_variable("word_embeddings",shape = shape, dtype=tf.float32, initializer=init) 
    ...
    def add_prediction_op(self):
        ...
        b2 = tf.get_variable("b1",shape=[self.config.n_classes],dtype=tf.float32, initializer= tf.zeros_initializer())
        h_0 = tf.constant(0.0, shape=[1,self.config.hidden_size], dtype=tf.float32)      
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        total_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=preds,labels=self.labels_placeholder)
        masked_losss = tf.boolean_mask(total_loss, self.mask_placeholder)
        loss = tf.reduce_mean(masked_losss)
        ### END YOUR CODE
        return loss
